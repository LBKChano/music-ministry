...
add column if not exists allow_member_multiple_roles_same_service boolean not null default false;
...
begin
  ...
  into allow_same_member_multiple_roles
  ...
  create temp table if not exists pg_temp.auto_assign_skipped (
    assignment_id uuid primary key,
    service_id uuid not null,
    service_date date not null,
    service_time text,
    ...
    unavailable_members jsonb not null default '[]'::jsonb
  ) on commit drop;
  ...
  truncate table pg_temp.auto_assign_skipped;
  ...
  loop
    ...
    loop
      ...
      same_service_conflicts_for_slot := 0;
      unavailable_members := '[]'::jsonb;
      skip_reason := 'No available member matched this slot';
      ...
      if not has_role_candidates then
        ...
        insert into pg_temp.auto_assign_skipped (
          assignment_id,
          service_id,
          service_date,
          service_time,
      ...
      select
        ...
        coalesce(
          jsonb_agg(
            jsonb_build_object(
              'member_id', unavailable_role_candidates.id,
              'name', coalesce(nullif(trim(unavailable_role_candidates.name), ''), unavailable_role_candidates.email),
              'email', unavailable_role_candidates.email
            )
            order by coalesce(nullif(trim(unavailable_role_candidates.name), ''), unavailable_role_candidates.email)
          ),
          '[]'::jsonb
        )
      into rejected_unavailable, unavailable_members
      from (
        ...
        where cm.church_id = target_church_id
          and cm.role is not null
          and lower(regexp_replace(trim(cm.role), '\s+', ' ', 'g')) = slot_normalized_role
      ) unavailable_role_candidates
      where exists (
        select 1
        from public.member_unavailability mu
        where mu.member_id = unavailable_role_candidates.id
      ...
      if not allow_same_member_multiple_roles then
        select count(*)::integer
      ...
      if selected_member_id is null then
        ...
        if coalesce(rejected_unavailable, 0) > 0 then
          ...
          skip_reason := 'Matching members are unavailable on this date';
        elsif not allow_same_member_multiple_roles and coalesce(same_service_conflicts_for_slot, 0) > 0 then
          skip_reason := 'Matching members are already assigned to this service';
        end if;
        ...
        values (
          ...
          slot_rec.role,
          skip_reason,
          coalesce(unavailable_members, '[]'::jsonb)
        );
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        'assignment_id', skipped.assignment_id,
        'service_id', skipped.service_id,
        'service_date', skipped.service_date,
        'service_time', skipped.service_time,
        'service_type', skipped.service_type,
        'role', skipped.role,
        'reason', skipped.reason,
        'unavailable_members', skipped.unavailable_members
      )
      order by skipped.service_date, skipped.service_time nulls last, skipped.service_type, skipped.role, skipped.assignment_id
    ),
    '[]'::jsonb
  )
  into skipped_report
  from pg_temp.auto_assign_skipped skipped;
