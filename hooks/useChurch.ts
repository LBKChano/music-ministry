...
export function useChurch() {
  ...
  // Fetch members for a specific church with their roles - FIXED
  const fetchMembers = useCallback(async (churchId: string) => {
    console.log('Fetching members for church:', churchId);
    try {
      setError(null);

      // Fetch members with their roles using a JOIN query
      const { data, error: fetchError } = await supabase
        .from('church_members')
        .select(`
          *,
      ...
      console.log('Fetched members with roles:', data);
      
      // Transform the data to match our interface
      const membersWithRoles: ChurchMemberWithRoles[] = (data || []).map(member => {
        const roleData = member.member_roles || [];
        const memberRoles = roleData
          .filter((mr: any) => mr.church_roles) // Filter out any null church_roles
          .map((mr: any) => ({
            role_id: mr.role_id,
            role_name: mr.church_roles.name,
          }));
      ...
      console.log('Transformed members with roles:', membersWithRoles);
  ...
  // Add role to member - FIXED: Check for existing role first
  const addMemberRole = useCallback(async (memberId: string, roleId: string, churchId: string) => {
    ...
    try {
      ...
      if (insertError) {
        // If it's a duplicate key error, treat it as success (race condition)
        if (insertError.code === '23505') {
          console.log('Role already assigned (race condition), treating as success');
          await fetchMembers(churchId);
          return true;
        }
  ...
  // Fetch current member info (for profile display) - FIXED
  const fetchCurrentMember = useCallback(async (churchId: string) => {
    ...
    try {
      ...
      // Fetch member with roles using JOIN
      const { data, error: fetchError } = await supabase
        .from('church_members')
        .select(`
          *,
      ...
      // Transform the data
      const roleData = data.member_roles || [];
      const memberRoles = roleData
        .filter((mr: any) => mr.church_roles)
        .map((mr: any) => ({
          role_id: mr.role_id,
          role_name: mr.church_roles.name,
        }));

      const currentMemberData: ChurchMemberWithRoles = {
        id: data.id,
      ...
      console.log('Current member with roles:', currentMemberData);
